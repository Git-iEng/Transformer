...
/* ===== CONFIG ===== */
const SOLAR = {
  revealSelector: '.reveal-transformer',
  inViewClass: 'in-view-transformer',
...
(function initLogosPager() {
  ...
  const track = document.getElementById('logos-track-transformer');
  const dotsWrap = document.getElementById('dots-transformer');
  if (!wrap || !track || !dotsWrap) return;

  const dots = Array.from(dotsWrap.querySelectorAll('.dot-transformer'));
  const RESUME_DELAY = 3500; // ms after click before continuous scroll resumes
  let resumeTimer = null;

  // set active dot helper
  function setActiveDot(idx) {
    dots.forEach((d, i) => d.classList.toggle('is-active-transformer', i === idx));
  }
  setActiveDot(0);
  ...
  function enterManualMode() {
    track.classList.add('manual-transformer');
  ...
  function resumeContinuous() {
    ...
    track.classList.remove('manual-transformer');
  ...
  const ro = new ResizeObserver(() => {
    const active = dots.findIndex(el => el.classList.contains('is-active-transformer'));
    if (active > -1 && track.classList.contains('manual-transformer')) {
  ...
  wrap.addEventListener('mouseenter', () => {
    if (!track.classList.contains('manual-transformer')) {
      track.style.animationPlayState = 'paused';
    }
  });
...
(function initImpactCounters() {
  const items = document.querySelectorAll('.stat-value-transformer-impact');
...
(function initSolutionsToggle() {
  const grid = document.getElementById('solutions-grid-transformer-solution');
  const btn = document.getElementById('solutions-toggle-btn-transformer-solution');
  if (!grid || !btn) return;

  function setState(expanded) {
    grid.classList.toggle('is-collapsed-transformer-solution', !expanded);
...
(function initProjectsCarousel() {
  const viewport = document.getElementById('projects-viewport-transformer-projects');
  const prevBtn = document.querySelector('.prev-transformer-projects');
  const nextBtn = document.querySelector('.next-transformer-projects');
...
(function initSolarTypes() {
  const tabs = Array.from(document.querySelectorAll('.tab-btn-transformer-types'));
  const panels = {
    'on-grid': document.getElementById('panel-on-grid-transformer-types'),
    'off-grid': document.getElementById('panel-off-grid-transformer-types'),
    'hybrid': document.getElementById('panel-hybrid-transformer-types')
  };
  ...
  function activate(type) {
    ...
    tabs.forEach(btn => {
      ...
      btn.classList.toggle('is-active-transformer-types', isActive);
    ...
    Object.entries(panels).forEach(([key, el]) => {
      const show = key === type;
      if (!el) return;
      el.classList.toggle('is-active-transformer-types', show);
      el.hidden = !show;
      if (show) {
  ...
  document.querySelector('.tabs-transformer-types')?.addEventListener('keydown', (e) => {
    const idx = tabs.findIndex(b => b.classList.contains('is-active-transformer-types'));
...
/* ==========================================================
   Scoped tabs for all .section-types-transformer-types
   (no global getElementById; supports multiple instances)
   ========================================================== */
(function initAllSolarTypeTabs() {
  document.querySelectorAll('.section-types-transformer-types').forEach(section => {
    const tabsWrap = section.querySelector('.tabs-transformer-types');
    if (!tabsWrap) return;

    const tabs = Array.from(section.querySelectorAll('.tab-btn-transformer-types'));
    const panels = Array.from(section.querySelectorAll('.panel-transformer-types'));
    ...
    function activate(btn) {
      ...
      tabs.forEach(t => {
        ...
        t.classList.toggle('is-active-transformer-types', isActive);
      ...
      panels.forEach(p => {
        ...
        p.hidden = !show;
        p.classList.toggle('is-active-transformer-types', show);
    ...
    tabsWrap.addEventListener('keydown', (e) => {
      ...
      e.preventDefault();
      const current = tabs.findIndex(t => t.classList.contains('is-active-transformer-types'));
      const dir = e.key === 'ArrowRight' ? 1 : -1;
    ...
    activate(tabs.find(t => t.classList.contains('is-active-transformer-types')) || tabs[0]);
  });
})();
...
(function () {
  const grid = document.getElementById('grid-transformer-card-with-animation');
  if (!grid) return;
  const cards = grid.querySelectorAll('.card-transformer-card-with-animation');

  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('show-transformer-card-with-animation');
      } else {
        // remove so it replays when scrolling back (LIFO feel)
        e.target.classList.remove('show-transformer-card-with-animation');
      }
    });
  }, { threshold: 0.18 });
...
(function () {
  const items = document.querySelectorAll('.reveal-transformer-panel-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-transformer-panel-');
  ...
  const form = document.getElementById('service-form-transformer-panel-');
...
(() => {
  const els = document.querySelectorAll('.reveal-transformer-panel-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-transformer-panel-');
...
(() => {
  const items = document.querySelectorAll('.reveal-transformer-about-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-transformer-about-');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const els = document.querySelectorAll(
    '.reveal-left-transformer-services, .reveal-right-transformer-services, .reveal-up-transformer-services'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-transformer-services');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const els = document.querySelectorAll(
    '.reveal-left-transformer-process, .reveal-right-transformer-process'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-transformer-process');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-left-transformer-why-us, .reveal-right-transformer-why-us, .reveal-top-transformer-why-us, .reveal-bottom-transformer-why-us'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('reveal-in-transformer-why-us');
...
special: {
  ...
  grid.addEventListener('click', (e) => {
    const btn = e.target.closest('.svc-cta-transformer-services');
    if (!btn) return;
  ...
  if (exploreBtn) {
    const allCards = Array.from(grid.querySelectorAll('.svc-item-transformer-services'));
    ...
    function setExpanded(state) {
      ...
      extraCards.forEach(el => {
        el.classList.remove('reveal-in-transformer-services');
        void el.offsetWidth;
      });
    }
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('reveal-in-transformer-services');
      } else {
        entry.target.classList.remove('reveal-in-transformer-services');
      }
    });
...
(() => {
  ...
  const close = modal.querySelector('.modal-close-csc-transformer-csc-products');
  ...
  document.querySelectorAll('.request-download-csc-transformer-csc-products').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  modal.querySelector('.modal-backdrop-csc-transformer-csc-products')
       .addEventListener('click', closeModal);
  document.addEventListener('keydown', e => { if (e.key === 'Escape') closeModal(); });
})();
(function(){
  // Reveal on scroll
  const els = document.querySelectorAll('.reveal-transformer-all-features');
  const io = new IntersectionObserver((entries)=>{
    entries.forEach(e=>{
      if(e.isIntersecting){
  ...
  document.querySelectorAll('.card-transformer-all-features').forEach(a=>{
